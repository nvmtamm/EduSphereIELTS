...
export function getAccentBadge(accent: string): { label: string; flag: string; bgClass: string; textClass: string } {
  switch (accent.toLowerCase()) {
    case 'british':
      return { label: 'British Accent', flag: '🇬🇧', bgClass: 'bg-red-500/10 border-red-500/20', textClass: 'text-red-600 dark:text-red-400' };
    case 'american':
...
export function getBandScoreColor(band: number): string {
  ...
  if (band >= 7.0) return 'text-red-600 dark:text-red-400 bg-red-500/10 border-red-500/30';
